...
def _normalize_for_embedding(value):
    if value is None:
        return ""
...
def evaluate_closedie(ground_truth_path, predicted_path, value_threshold=0.5):
    ...
    for filename in tqdm(common_files):
        ...
        for gt_key, gt_vals in gt_entities.items():
            gt_values_raw = gt_vals if isinstance(gt_vals, list) else [gt_vals]
            gt_values = [_normalize_for_embedding(v) for v in gt_values_raw if v is not None]
            pred_raw = pred_entities.get(gt_key, [])
            pred_raw_list = pred_raw if isinstance(pred_raw, list) else [pred_raw]
            pred_values = [_normalize_for_embedding(v) for v in pred_raw_list if v is not None]
            if not pred_values:
...
def evaluate_openie(ground_truth_path, predicted_path, key_threshold=0.5, value_threshold=0.5):
    ...
    for filename in tqdm(common_files):
        ...
        for gt_key, gt_value in gt_entities.items():
            ...
            if best_pred_key:
                pred_value_raw = pred_entities[best_pred_key]
                pred_value = _normalize_for_embedding(pred_value_raw)
                gt_value_norm = _normalize_for_embedding(gt_value)
                value_score, value_match = embedding_sim_sbert(
                    gt_value_norm, pred_value, model, value_threshold
                )
                if value_match:
                    entity_metrics[gt_key]["tp"] += 1
                else:
